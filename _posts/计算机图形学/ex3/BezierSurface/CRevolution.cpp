...
void CRevolution::ReadCubicBezierControlPoint(CP3 P[4])//三次Bezier 曲线4 个控制点初始化
{
	for (int i = 0; i < 4; i++)
		this->P[i] = P[i];
	//读入回转体的数据结构
...
void CRevolution::ReadVertex(void)//读入回转体的所有控制点
{
	...
	//回转一圈需要4 个面,第一个面16 个点,第二个面12 个点，第三个面12 个点，第四个面8 个点，
