...
// CZBufferView.cpp: CCZBufferView 类的实现
//
...
#include "CZBufferDoc.h"
#include "CZBufferView.h"
...
IMPLEMENT_DYNCREATE(CCZBufferView, CView)

BEGIN_MESSAGE_MAP(CCZBufferView, CView)
	// 标准打印命令
	...
	ON_WM_CONTEXTMENU()
	ON_WM_RBUTTONUP()
	ON_COMMAND(ID_GRAPHIC_ANIMATION, &CCZBufferView::OnGraphicAnimation)
	ON_UPDATE_COMMAND_UI(ID_GRAPHIC_ANIMATION, &CCZBufferView::OnUpdateGraphicAnimation)
	ON_WM_KEYDOWN()
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
CCZBufferView::CCZBufferView() noexcept
{
	// TODO: 在此处添加构造代码
	bPlay = FALSE;
	double R = 400;
...
BOOL CCZBufferView::PreCreateWindow(CREATESTRUCT& cs)
{
	// TODO: 在此处通过修改
...
void CCZBufferView::OnDraw(CDC* pDC)
{
	CCZBufferDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	if (!pDoc)
		return;
...
void CCZBufferView::DoubleBuffer(CDC* pDC)//双缓冲
{
	CRect rect;//定义客户区矩形
...
void CCZBufferView::DrawObject(CDC* pDC)//绘制图形
{
...
BOOL CCZBufferView::OnPreparePrinting(CPrintInfo* pInfo)
{
	// 默认准备
	return DoPreparePrinting(pInfo);
}

void CCZBufferView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: 添加额外的打印前进行的初始化过程
}

void CCZBufferView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: 添加打印后进行的清理过程
}
...
#ifdef _DEBUG
void CCZBufferView::AssertValid() const
{
	CView::AssertValid();
}

void CCZBufferView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}

CCZBufferDoc* CCZBufferView::GetDocument() const // 非调试版本是内联的
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CCZBufferDoc)));
	return (CCZBufferDoc*)m_pDocument;
}

void CCZBufferView::InitializeLightingScene(void)//初始化光照环境
{
	//设置光源属性
...
// CCZBufferView 消息处理程序

void CCZBufferView::OnGraphicAnimation()
{
	// TODO: 在此添加命令处理程序代码
	bPlay = !bPlay;
	if (bPlay)//设置定时器
...
void CCZBufferView::OnUpdateGraphicAnimation(CCmdUI* pCmdUI)
{
...
void CCZBufferView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CCZBufferView::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	Alpha = 5, Beta = 5;
	tranUp.RotateX(Alpha);
